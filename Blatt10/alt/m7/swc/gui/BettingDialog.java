...
public class BettingDialog extends JDialog {
	...
	private void drawMatches() {
		...
		for (int j = 0; j < 5; j++) {
			...
			for (int i = 0; i < data.length; i++) {
				...
				data[i][5] = "" + currentGames.get(i).getTeamH().getName();
				if (currentGames.get(i).isPlayed())
					data[i][6] = currentGames.get(i).getGoalsH() + " - " + currentGames.get(i).getGoalsG();
				else
					data[i][6] = " - ";
				data[i][7] = "" + currentGames.get(i).getTeamG().getName();
				try {
					data[i][4] = CtrlGroup.getFlagIcon(currentGames.get(i).getTeamH().getName());
					data[i][8] = CtrlGroup.getFlagIcon(currentGames.get(i).getTeamG().getName());
	...
	/**
	 * Function to download already made tips by the student
	 * 
	 * @param betterEmail email of the student who is betting
	 * @return retruns vector which includes the tip the student made
	 */
	private Vector<Tip> downloadTipsFromServer(String betterEmail) {
		Vector<Tip> tips = new Vector<Tip>();
		String urlString = "http://swc.dbis.info/api/Betting/" + betterEmail;
		try {
			...
			for(JsonObject data : results.getValuesAs(JsonObject.class)) {
				tips.add(new Tip(data.getInt("gameId"),
						data.getInt("goalsHome"),
						data.getInt("goalsGuest")));		
			}
			rdr.close();
		}
		catch(Exception e) {
	...
	/**
	 * Function to upload new tips made by a student to the server
	 * @param tips Vector which includes the tip the student made
	 * @param betterEmail user eMail address
	 * @param betterPin user PIN for authentication 
	 */
	private void uploadTipsToServer(Vector<Tip> tips, String betterEmail, String betterPin) {
		for(Tip data : tips) {
			String urlString = "http://swc.dbis.info/api/Betting/" + betterEmail + "/" + betterPin;
			urlString += "/" + String.valueOf(data.getGameId());
			urlString += "/" + String.valueOf(data.getGoalsHome());
			urlString += "/" + String.valueOf(data.getGoalsGuest());
		...
		try {
			...
			JsonValue check = rdr.readValue();
			if(!Boolean.parseBoolean(check.toString())) {
				System.out.println("ERROR");
				return;
			}
			if(!Boolean.parseBoolean(check.toString()))
				JOptionPane.showMessageDialog(this, "Something went wrong. You probably entered the wrong PIN or wrong E-Mail","ERROR",JOptionPane.ERROR_MESSAGE);
		}
		catch(Exception e) {
			e.printStackTrace();
		}
		}
		JOptionPane.showMessageDialog(this, "Tips uploaded succesfully", "Done", JOptionPane.INFORMATION_MESSAGE);
