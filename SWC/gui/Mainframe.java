...
public class Mainframe extends JFrame implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		 ...
		 */
		else if(action.equals("Load from server...")){
			loadWorldCupFromServer();
		}
		/*
		...
		if(option == JFileChooser.APPROVE_OPTION) {
			try {
				...
				if(ending.equals("xml"))
					newWorldCup = XMLFiles.readFromXML(filename, worldCup);
				else
	...
	/**
	 * Loads a world cup from a remote destination.
	 */
	private void loadWorldCupFromServer(){
		String path = LoadServerDialog.getOption(this);
		if(path == null)
			return;
		try {
			listener.setUpdateFinals(false);
			SoccerWC newWorldCup = XMLFiles.readFromRemoteFile(path, worldCup);
			this.worldCup = newWorldCup;
			listener.setWorldCup(worldCup);
			CtrlFinals.calculateFinals(newWorldCup);
	...
	private void saveWorldCup() {
		...
		if(worldCup.getFilename().equals(""))
			...
			try {
				...
				String ending = path[path.length-1];
				if(ending.equals("xml"))
					XMLFiles.writeToXML(worldCup.getFilename(), worldCup);
				else
	...
	private void saveWorldCupToNewLocation() {
		...
		try {
			...
			String ending = path[path.length-1];
			if(ending.equals("xml"))
				XMLFiles.writeToXML(fileName, worldCup);
			else
